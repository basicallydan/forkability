(function() {
	...
	function forkability(options, callback) {
		...
		var recursiveGetFileTrees = function(response, tree){
			...
			console.log(recursing + " many times");
			recursing++;
			...
			if (recursing === 3) {
				var MMtree = response[1].tree;
				console.log('my fault');
				console.log(MMtree);
				var report = lintFiles(MMtree, lintOptions);
				passes = passes.concat(report.passes);
				failures = failures.concat(report.failures);
				return response.then(get('https://api.github.com/repos/' + username + '/' + repo + '/issues?state=open'));
				// return response;
			};
			...
			console.log('Nothing was returned from the for');
			return response.then(recursiveGetFileTrees(response));
		};
		...
		get('https://api.github.com/repos/' + username + '/' + repo + '/commits')
			...
			.then(function(response) {
				...
				// return get(url).then(recursiveGetFileTrees);
				return get(url);
			})
			.then(recursiveGetFileTrees)
			// .then(function(response){
