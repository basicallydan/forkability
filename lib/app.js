(function() {
	...
	function forkability(options, callback) {
		...
		var recursiveGetFileTrees = function(response, tree){
			...
			if (tree) {
				localTree = tree;
			} else if (fileWithTrees.length === 0) {
				fileWithTrees = localTree;
			};

			if (recursing === options.fileDepth) {
				var resultTree = response[1].tree;
				var report = lintFiles(fileWithTrees, lintOptions);
				...
				if (localTree[item].type === 'tree' && localTree[item].url && !localTree[item].tree) {
					...
					return get(localTree[item].url)
							.then(function(recursiveResponse){
									...
									return recursiveGetFileTrees(recursiveResponse, recursiveResponse[1].tree);
							});
				}
			}
