module.exports = function(issues, username) {
	...
	issues.forEach(function(issue) {
		if (issue.user.login.toLowerCase() !== username.toLowerCase() && issue.comments === 0) {
			failures.push({
		...
		if (issue.user.login.toLowerCase() !== username.toLowerCase() && issue.labels.length === 0 && issue.comments === 0) {
			failures.push({
